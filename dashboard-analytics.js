...
function renderDataHealth(){
  ...
  alert.textContent=failed.length?'Не обновились: '+failed.map(function(key){return names[key];}).join(', ')+'. Зависимые показатели недоступны.':'Данные загружены. МК учтены по отчётам об уроках и первичным оплатам, без повторов между источниками.';
...
function renderMain(){
  ...
  var selection=analyticsSelection(),t=selection.totals, payOk=analyticsReady(['payments']), mkOk=analyticsReady(['form','payments']);
  ...
  document.getElementById('ks-mk').textContent='Отчёты МК + первичные оплаты без повтора урока';
...
function renderProg(){
  ...
  document.getElementById('cc-mk').textContent=analyticsMetric(planOk&&analyticsReady(['form','payments'])?Math.max(0,targets.mk-t.mk):null,' МК');
  ...
  el.innerHTML=[{label:'МК за месяц',value:analyticsReady(['form','payments'])?t.mk:null,target:targets.mk},{label:'Выручка за месяц',value:analyticsReady(['payments'])?t.rev:null,target:targets.rev}].map(function(row){var ratio=row.value!=null&&row.target>0?row.value/row.target*100:null;return '<div class="pr"><span class="pl">'+row.label+'</span><div class="pt2"><div class="pf g" style="width:'+Math.max(0,Math.min(100,ratio||0))+'%"></div></div><span class="pp">'+analyticsMetric(ratio,'%')+'</span></div>';}).join('');
}
...
function renderKpiSparks(){
  ...
  ['rev','mk','conv','avg','rpm'].forEach(function(key){var ready=analyticsReady(['form','payments']),el=document.getElementById('spark-'+key);el.innerHTML=ready&&rows.length>1?sparkPath(rows.map(function(row){return row[key]||0;}),68,22):'';});
}
function shareReport(){
  ...
  lines.push('','МК: отчёты об уроках + первичные оплаты без отдельного отчёта, по одной сделке один оплаченный МК. Если дата урока известна, используется она; иначе дата первой первичной оплаты. Продления не добавляют МК. Конверсия = оплаты периода / МК периода.');
